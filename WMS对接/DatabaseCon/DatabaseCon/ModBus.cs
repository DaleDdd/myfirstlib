...
namespace DatabaseCon
{
    class ModBus
    {
        ...
        public static bool Poll_Send = false;
        #region 构造函数
        /// <summary>
        /// 构造函数
        /// </summary>
        public ModBus()
        {
            ...
            //串口参数赋值
            sp.PortName = "COM3";
            sp.BaudRate = 115200;     //波特率115200
            sp.DataBits = 8;
            sp.Parity = Parity.None;
            sp.StopBits = StopBits.One;
            // sp.ReceivedBytesThreshold = 1;
            //判断是否开启，没开启直接打开
            Thread.Sleep(2000);                 //延迟2s，等待驱动安装完成
            if (!sp.IsOpen)
            {
                try
                {
        ...
        #endregion
        #region 属性
        public int station { get; set; }   //站点工位
        ...
        public void SendBuf(ref byte[] buf)
        {
        ...
        private void HandleData(object o)
        {
            ...
            if(0==crc && buff[0]==0xF7 && buff[1]==0x01)
            {
                if(buff[2]==0x01)        //接收到了AGV启动的回应
                {
                    RecivedMessage("AGV启动");
                }
                if(buff[2]==0x02&&buff[3]==0x00)        //接收到了AGV返回站点的命令
                {
                    byte[] send_return = new byte[] { 0xF7, 0x01, 0x02, 0x00 };
                    SendBuf(ref send_return);           //回应AGV
                    RecivedMessage("AGV返回");
                }
            }
        }
        /// <summary>
